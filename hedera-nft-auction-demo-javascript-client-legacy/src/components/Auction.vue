  ...
  <div>
    <v-card :color="cardColor" class="mx-auto ma-1" outlined width="650px">
      <v-toolbar :color="cardColor" dark>
        <v-toolbar-title v-if="tokenURL" class="white--text"
          >Auction for token
          <a
            :href="tokenURL"
        ...
        <v-toolbar-title v-else class="white--text"
          >Auction for token <b>{{ tokenid }}</b></v-toolbar-title
        >
      </v-toolbar>
      <v-row align="center" v-if="status === 'ACTIVE'">
        <flip-countdown
          class="ma-2"
          :deadline="timeFromSeconds(endtimestamp)"
        ></flip-countdown>
      </v-row>
      <v-row align="center" class="ma-2">
        ...
        <v-col align="left">
          <v-row>
            <v-col>
              <a
                ...
                style="text-decoration: none; color: inherit;"
                target="_blank"
                ><b>Auction account: {{ auctionaccountid }}</b></a
              >
              <a v-else style="text-decoration: none; color: inherit;"
                >Auction account: <b>{{ auctionaccountid }}</b></a
              >
            </v-col>
          </v-row>
          <v-row>
          ...
          <v-row v-if="status === 'ENDED'">
            <P v-if="transferTransactionURL"
              >on
              <a
                :href="transferTransactionURL"
                style="text-decoration: none; color: inherit;"
                target="_blank"
                ><b>Token Transfer Transaction</b></a
              ></P
            >
            <P v-else
              ><b>Token Transfer {{ transfertxid }}</b></P
            >
          </v-row>
        </v-col>
      </v-row>
