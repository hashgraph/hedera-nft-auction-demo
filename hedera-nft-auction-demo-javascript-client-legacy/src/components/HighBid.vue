  ...
  <div>
    ...
    <div v-if="status !== 'TESTING'">
      <v-card class="ma-1" outlined width="650px">
        <v-toolbar dark>
          <v-toolbar-title v-if="status !== 'PENDING'" class="white--text"
            >Highest Bid</v-toolbar-title
          >
        </v-toolbar>
        <v-card-text>
          <v-sheet elevation="1" height="100%" width="100%" outlined rounded>
            <v-row align="center">
              <v-col>
                ...
                <div v-if="winningaccount">
                  <P v-if="accountURL"
                    ...
                    <a
                      ...
                      target="_blank"
                      ><b>{{ winningaccount }}</b></a
                    ></P
                  >
                  <P v-else
                    >by <b>{{ winningaccount }}</b></P
                  >
                  <P v-if="winningTxURL"
                    >on
                    <a
                      ...
                      style="text-decoration: none; color: inherit;"
                      target="_blank"
                      ><b>{{ timeFromSeconds(winningtimestamp) }}</b></a
                    ></P
                  >
                  <P v-else
                    >on <b>{{ timeFromSeconds(winningtimestamp) }}</b></P
                  >
                </div>
                <div v-else>
                  <p class="ma-6 display-1 font-weight-bold text--white">
